...
function resetGame() {
  waitTimer = null;
  cards.value = generateCards(totalCount.value, props.matchCount);
...
function onFlip(index) {
  ...
  const allFlippedAreSame = flipped.value.every(
    (index) => cards.value[flipped.value[0]] === cards.value[index]
  );
  ...
  if (allFlippedAreSame) {
    ...
    if (matched.value.size * props.matchCount === totalCount) {
...
function checkDisabledCard(index) {
  return matched.value.has(cards.value[index]) || flipped.value.includes(index);
}

watch([() => props.cols, () => props.rows, () => props.matchCount], resetGame, {
  immediate: true,
});
</script>
  ...
  <div class="app">
    ...
    >
      <button
        ...
        :key="index"
        :class="['card', matched.has(cards[index]) && 'card--revealed']"
