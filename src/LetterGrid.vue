...
function checkIntermediateState(state) {
    ...
    <template v-for="(letterRow, rowIndex) in letters">
      <button
        ...
        :class="{
          ...
          'grid-cell--final': !checkIntermediateState(state),
        }"
        :style="{
          'transition-delay': !checkIntermediateState(state)
            ? `${colIndex * 50}ms`
            : undefined,
        }"
      >
