...
const gridState = ref(
  getInitialGridState(props.maxAttempts, props.lettersPerWord)
);
...
function checkWord() {
  ...
  const word = gridState.value[row].map(({ char }) => char).join("");
  // Create a map of count of letters in original word to compare
  // with the word entered by user.
  const letterFreq = countLetterFreqInWord(wordOfTheDay.value);
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (currentChar === currentActualChar) {
      gridState.value[row][i].state = LETTER_GUESS_STATE.CORRECT;
      letterGuessState.value[currentChar] = LETTER_GUESS_STATE.CORRECT;
  ...
  nonMatchingIndices.forEach((idx) => {
    ...
    if (letterFreq.has(char) && letterFreq.get(char) > 0) {
      ...
      gridState.value[row][idx].state = LETTER_GUESS_STATE.PRESENT;
      // Only change state if the new state is higher priority
      // than the current state.
      if (
        LETTER_GUESS_STATE_PRIORITY[LETTER_GUESS_STATE.PRESENT] >
        LETTER_GUESS_STATE_PRIORITY[
          letterGuessState.value[char] ?? LETTER_GUESS_STATE.INDETERMINATE
        ]
      ) {
        letterGuessState.value[char] = LETTER_GUESS_STATE.PRESENT;
      }
    } else {
      gridState.value[row][idx].state = LETTER_GUESS_STATE.ABSENT;
      // Only change state if the new state is higher priority
      // than the current state.
      if (
        ...
        letterGuessState.value[char] = LETTER_GUESS_STATE.ABSENT;
      }
    }
  });
...
watchEffect(() =>
  console.log(`[DEBUG]: Word of the day is: ${wordOfTheDay.value}`)
);

watch([() => props.maxAttempts, () => props.lettersPerWord], () => {
  resetGame();
});
</script>
...
<template>
  <main class="root">
    ...
    <div v-if="gameState !== GAME_STATE.IN_PROGRESS" class="result-row">
      <strong>
        {{
    ...
    <Keyboard @press-key="onPressKey" :letterGuessState="letterGuessState" />
  </main>
</template>
